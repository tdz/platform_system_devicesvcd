...
function WifiManager() {
  ...
  this.socket = new WebSocket('http://localhost:80', 'WifiManager');
  this.socket.onmessage = function(message) {
      if (this.commandQueue.length && message.type == this.commandQueue[0]) {
        // reply
        var cmd = this.commandQueue.shift();
        this.handleReply(message, cmd.onsuccess, cmd.onerror);
      } else {
        // notification
